...
index = 5
semantic = 'report date of the crime'
data_type = 'DATE'
    ...
    if date == '':
        ...
        try:
            ...
            if year < 2006 or year > 2015:
                raise ValueError
            # report date before starting date
            if row[1] != '' and datetime.strptime(row[1], "%m/%d/%Y") > datetime.strptime(date, "%m/%d/%Y"):
                raise ValueError
            # report date before ending date
            if row[3] != '' and datetime.strptime(row[3], "%m/%d/%Y") > datetime.strptime(date, "%m/%d/%Y"):
                raise ValueError
        except ValueError:
            value = 'INVALID'
